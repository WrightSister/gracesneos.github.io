...
function previewListing(fullListing) {
  ...
  var cssString = `<center><form id = "userInput" style="width: 25%"><fieldset><input type="text" id=userName" name="userName" value = "${user}" style="margin-bottom: 0px"></fieldset></form></center><br>`; // Starting CSS preview
  ...
  for (let i = 0; i < pets.length; i++) {
    cssString += `<div id = "subShowListing"><form id = "form${i}">
      <fieldset>
        ...
        <input type="text" id="petName" name="petName${i}" value="${pets[i].petName}"><br>
        <label for="petColor">Color:</label><br>
        <input type="text" id="petColor" name="petColor${i}" value="${pets[i].color}"><br>
        <label for="petSpecies">Species:</label><br>
        <input type="text" id="petSpecies" name="petSpecies${i}"  value="${pets[i].species}"><br>
        <label for="petExtra">Extra details:</label><br>
        <input type="text" id="petExtra" name="petExtra${i}" placeholder="(Optional) (RW), (HSD), etc." value="${pets[i].extra}"><br>
        <label for="petDescription">Description:</label><br>
        <input type="text" id="petDescription" name="petDescription${i}" placeholder="(Optional) Funny joke goes here"></div>

      <div id="subForm" style="text-align: left">
        <center><p><label for="initiate">Can't initiate?</label></p><input type="checkbox" id="check${i}">
  ...
  cssString += `<br><button id="addListing" onclick="addListing()">Add Listing</button>` // Button for finalizing listing
...
function addListing() {
  ...
  container.innerHTML = `Listing added. Add another in the listing box or generate petpage if done.
<br><button id="genPetpage" onclick="genPetpage()">Generate Petpage</button>`
...
<center>
Coding/Graphics by Sunny @ <a href="/~Protester">Resource</a>, specifically for distribution on <a href="/~bydz">Bydz's PAH Resource Page</a>.<br><br></center> 
...
<li>BN/VBN, Basics, Compounds, Uncapped and 2/3C pets are invalid bids.</li>`).replace("BANNERURL", "http://i.imgur.com/QmeMK1x.png");
  ...
  container.innerHTML = `<b>Template:</b><br><textarea id="showCode">hi</textarea>`
